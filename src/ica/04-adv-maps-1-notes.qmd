...
mn_cities <- st_read("~/Documents/GitHub/portfolio-my-name-arch/src/ica/metadata/city_and_township_population_centers.shp")
library(sf)
mn_water <- ("~/Documents/GitHub/portfolio-my-name-arch/src/ica/metadata/LakesAndRivers.shp")
```
...
```{r transform_nc}
nc_transformed <- nc |> st_transform(crs = "EPSG:2991")
...
transform_and_plot <- function(spatial_obj, new_crs, plot_title) {
    spatial_obj |>
        st_transform(crs = new_crs) |>
        ggplot() +
        geom_sf() + # The main geometry layer
        ggtitle(plot_title) + # Use the new argument for the title
        theme_classic()
}
transform_and_plot(nc,"EPSG:2991","Oregon")
...
library(ggthemes)
ggplot() +
  # First geom_sf() call: draw the county boundaries
  geom_sf(data = mn_counties, fill = "#F5F5DC", color = "gray70", linewidth = 0.25) +
  
  # Second geom_sf() call: draw the city points on top
  geom_sf(data = mn_cities, color = "#B22222", size = 3.5, shape = 18) +
  # Apply the clean map theme
  theme_map() +
  
  # Add titles
  labs(title = "Major Cities and County Boundaries in Minnesota",
...
It improves accessibility for those who are colorblind
- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?
I takes types of scale whether it's continuous, discrete, and binned
...
```{r}
ggplot() +
  # Layer 1: Elevation raster background
  geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) +
  
  # Layer 2: County boundaries (transparent fill)
  geom_sf(data = mn_counties, fill = NA, color = "gray50") +
  
  # Layer 3: City points, filtered for population >= 10,000
  geom_sf(data = mn_cities |> filter(Population >= 10000), 
          mapping = aes(color = Population, size = Population)) +
          
  # Custom color scales
  scale_fill_gradient(low = "darkgreen", high = "white", name = "Elevation") +
  scale_color_viridis_c(name = "City Population") +
  
  # Apply a clean map theme
  ggthemes::theme_map() +
  
  # Add a title and adjust legend
  labs(title = "Minnesota Cities (Pop. >= 10,000) by Elevation") +
...
output_path <- "figures/tc_map_zoom.png"

# Start the PNG graphics device
png(output_path, width = 800, height = 800)

# --- Code for creating the plot (from the prompt) ---
ggplot() +
    geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + 
    geom_sf(data = mn_counties, fill = NA, color = "black") + # county boundary layer
    geom_sf(data = mn_water, fill = "lightsteelblue1", color = "lightsteelblue1") + # river/lake layer
    geom_sf(data = mn_cities |> filter(Population >= 10000), mapping = aes(color = Population, size = Population)) + # cities layer
    ...
    coord_sf(xlim = seven_countyarea[c("xmin", "xmax")], ylim = seven_countyarea[c("ymin", "ymax")]) + # crop map
