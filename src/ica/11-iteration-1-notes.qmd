...
month_strings_loop <- character(length(month.name))

for (i in seq_along(month.name)) {
  # Use paste0 for efficient string concatenation
  month_strings_loop[i] <- paste0("Month ", i, ": ", month.name[i])
}
print(month_strings_loop)


# vectorized approach with `str_c()`
month_strings_vectorized <- str_c("Month ", seq_along(month.name), ": ", month.name)
print(month_strings_vectorized)
...
```{r}
# for loop
print("Fitting models of price vs. carat for each cut:")
# Get the unique values of the 'cut' variable
cut_levels <- unique(diamonds$cut)
# Initialize an empty list to store the fitted models
model_list <- list()

for (cut_value in cut_levels) {
  # Subset the data for the current cut level
  subset_data <- subset(diamonds, cut == cut_value)
  
  # Fit a linear model of price vs. carat on the subset
  fit <- lm(price ~ carat, data = subset_data)
  
  # Store the model in the list, naming the element by the cut value
  # Note: as.character() is used to ensure the name is a simple string
  model_list[[as.character(cut_value)]] <- fit
}

# You can inspect the list of models
print("Summary of the model for 'Ideal' cut diamonds:")
summary(model_list$Ideal)

print("List of all fitted models:")
print(model_list)
...
# 1. Argument Combinations
df_arg_combos <- crossing(
    mod_formula = c(price ~ carat, price ~ carat + cut,  price ~ carat + cut + clarity,  price ~ carat + cut + clarity + color),
    remove_outliers = c(TRUE, FALSE)
)

# 2. remove_outliers function
remove_outliers <- function(data, what_var, sd_thresh) {
    data |> 
        mutate(zscore = ({{ what_var }} - mean({{ what_var }}, na.rm = TRUE))/sd({{ what_var }}, na.rm = TRUE)) |>
        filter(abs(zscore) <= sd_thresh) # Added abs() to filter on both sides
}

# 3. Corrected fit_model function
fit_model <- function(data, mod_formula, remove_outliers) {
    if (remove_outliers) {
        data_clean <- remove_outliers(data, what_var = carat, sd_thresh = 3)
    } else {
...
# 4. The `for` loop to fit and store the models
...
for (i in seq_along(lin_mod_res_for)) {
    ...
    # The arguments now correctly match the fit_model function signature
...
# Print the first model to verify the loop ran successfully
print("The first fitted model (No outliers, Formula: price ~ carat):")
print(summary(lin_mod_res_for[[1]]))

print(str(lin_mod_res_for))
```
## Done!
