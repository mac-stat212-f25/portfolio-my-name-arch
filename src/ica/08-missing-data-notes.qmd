...
1. **Missing data mechanism** For each of the following situations, propose what missing data mechanism you think is most likely at play. 
  a. In a clinical trial, some patients dropped out before the end of the study. Their reasons for dropping out were not recorded. MCAR
  
  b. A weather station records temperature, humidity, and wind speed every hour. Some of the recorded values are missing. MAR
  
  c. A social media platform collects data on user interactions, such as likes, comments, and shares. Some interactions are not recorded due to bugs in the code.MNAR
...
I do think that a linear regression could be ffective
...
# 1a. Check if Ozone missingness is related to Month (MAR check)
airquality |> 
    ggplot(aes(x = Month, fill = is.na(Ozone))) +
    geom_bar(position = "fill") +
    labs(
        title = "Ozone Missingness by Month",
        y = "Fraction of Cases",
        fill = "Ozone is NA"
    ) +
...
# --- Investigation 2: Wind ---

# 2a. Check if Ozone missingness is related to Wind (MAR check)
airquality |> 
    ggplot(aes(x = is.na(Ozone), y = Wind)) + 
    geom_boxplot() +
    labs(
        title = "Wind distribution by Ozone missing status",
        x = "Ozone is NA",
        y = "Wind Speed"
    )

# 2b. Check for extrapolation in Wind
airquality |> 
    ggplot(aes(x = Wind, y = Ozone)) +
    geom_point() +
    geom_smooth(method = "lm") +
    geom_vline(data = airquality |> filter(is.na(Ozone)), 
               mapping = aes(xintercept = Wind), 
               color = "red", 
               alpha = 0.2) + # Vertical lines for Wind values where Ozone is NA
    labs(title = "Ozone vs. Wind, with NA Wind values marked")


# --- Investigation 3: Temp ---

# 3a. Check if Ozone missingness is related to Temp (MAR check)
airquality |> 
    ggplot(aes(x = Temp, color = is.na(Ozone))) + 
    geom_density() +
    labs(
        title = "Temperature distribution by Ozone missing status",
        color = "Ozone is NA"
    )

# 3b. Check for extrapolation in Temp
airquality |> 
    ...
    geom_vline(data = airquality |> filter(is.na(Ozone)), 
               ...
               color = "red", 
               alpha = 0.2) + # Vertical lines for Temp values where Ozone is NA
    labs(title = "Ozone vs. Temp, with NA Temp values marked")
```
